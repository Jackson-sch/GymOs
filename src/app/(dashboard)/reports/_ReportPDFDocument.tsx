    ...
    <Document>
      <Page size="A4" style={styles.page}>
        ...
        <View style={styles.section}>
          ...
          <PdfGraph variant="bar" data={revenueData} height={180} fullWidth colors={["#8b5cf6"]} />
        </View>
        ...
        <View style={styles.chartContainer} wrap={false}>
          <View style={styles.chartHalf}>
            ...
            <PdfGraph variant="donut" data={planData} width={240} height={180}
          ...
          <View style={styles.chartHalf}>
            ...
            <PdfGraph variant="donut" data={statusData} width={240} height={180}
        ...
        <View style={styles.section}>
          ...
          <View style={styles.heatmapContainer}>
            ...
            {heatmapData.map((row, hIndex) => (
              <View key={hours[hIndex]} style={styles.heatmapRow}>
                <Text style={styles.label}>{hours[hIndex]}</Text>
                {row.map((val, dIndex) => {
                  const intensity = val / maxAttendance;
                  return (
                    <View key={`${hours[hIndex]}-${days[dIndex]}`}
                      style={[styles.heatmapCell, { backgroundColor: `rgba(139, 92, 246, ${Math.max(0.05, intensity)})` }]}
                    />
                  );
                })}
              </View>
          ...
          <Table variant="line">
            ...
            <TableBody>
              {topMembers.slice(0, 5).map((m, i) => (
                <TableRow key={m.id || m.dni || m.fullName}>
